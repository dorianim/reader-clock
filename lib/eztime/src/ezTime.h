...
// Uncomment one of the below to only put only messages up to a certain level in
// the compiled code (You still need to turn them on with setDebug(someLevel) to
// see them) #define EZTIME_MAX_DEBUGLEVEL_NONE #define
// EZTIME_MAX_DEBUGLEVEL_ERROR #define EZTIME_MAX_DEBUGLEVEL_INFO
...
// Defines that can make your code more readable. For example, if you are
// looking for the first Thursday in a year, you could write:
// time.makeOrdinalTime(0, 0, 0, JANUARY, FIRST, THURSDAY, year)
...
typedef void (*ntpUpdateHandler_t)(void *args);
#endif
...
#define LAST_READ                                                              \
  (int32_t)0x7FFFFFFE // (So yes, ezTime might malfunction two seconds before
                      // everything else...)
...
#define NTP_STALE_AFTER                                                        \
  3602 // If update due for this many seconds, set timeStatus to timeNeedsSync
...
#define MAX_CACHE_PAYLOAD                                                      \
  ((EEPROM_CACHE_LEN - 3) / 3) * 4 +                                           \
      ((EEPROM_CACHE_LEN - 3) %                                                \
       3) // 2 bytes for len and date, then 4 to 3 (6-bit) compression on rest
...
time_t compileTime(const String compile_date = __DATE__,
...
time_t makeOrdinalTime(const uint8_t hour, const uint8_t minute,
                       const uint8_t second, uint8_t ordinal,
...
public:
  ...
  uint16_t dayOfYear(time_t t = TIME_NOW,
                     const ezLocalOrUTC_t local_or_utc = LOCAL_TIME);
  int16_t getOffset(time_t t = TIME_NOW,
                    const ezLocalOrUTC_t local_or_utc = LOCAL_TIME);
  String getPosix();
  ...
  bool isAM(time_t t = TIME_NOW,
            const ezLocalOrUTC_t local_or_utc = LOCAL_TIME);
  bool isDST(time_t t = TIME_NOW,
             const ezLocalOrUTC_t local_or_utc = LOCAL_TIME);
  bool isPM(time_t t = TIME_NOW,
            const ezLocalOrUTC_t local_or_utc = LOCAL_TIME);
  String militaryTZ(time_t t = TIME_NOW,
                    const ezLocalOrUTC_t local_or_utc = LOCAL_TIME);
  ...
  uint8_t second(time_t t = TIME_NOW,
                 const ezLocalOrUTC_t local_or_utc = LOCAL_TIME);
  uint8_t setEvent(void (*function)(), const uint8_t hr, const uint8_t min,
                   const uint8_t sec, const uint8_t day, const uint8_t mnth,
                   uint16_t yr);
  uint8_t setEvent(void (*function)(), time_t t = TIME_NOW,
                   const ezLocalOrUTC_t local_or_utc = LOCAL_TIME);
  bool setPosix(const String posix);
  void setTime(const time_t t, const uint16_t ms = 0);
  void setTime(const uint8_t hr, const uint8_t min, const uint8_t sec,
               const uint8_t day, const uint8_t mnth, uint16_t yr);
  time_t tzTime(time_t t = TIME_NOW, ezLocalOrUTC_t local_or_utc = LOCAL_TIME);
  time_t tzTime(time_t t, ezLocalOrUTC_t local_or_utc, String &tzname,
...
// The following defines all copied from the original Time lib to keep existing
// code working
...
#define previousMidnight(_time_)                                               \
  ((_time_ / SECS_PER_DAY) * SECS_PER_DAY) // time at the start of the given day
