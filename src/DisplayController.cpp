...
DisplayController::DisplayController()
    : _quoteFont{&FreeSerif12pt7b}, _boldFont{&FreeSerifBold12pt7b} {
  // GDEW027W3 2.7" b/w
  ...
  this->_display = new GxEPD_Class(
      *io, /*RST=*/16, /*BUSY=*/4); // arbitrary selection of (16), 4
#endif

  this->_display->init(115200, true, 2, false);
  this->_display->setTextWrap(false); // wrapping is done manually
  this->_display->setRotation(0);
...
void DisplayController::showQuote(Quote *quote) {
  ...
  this->_printAuothorAndTitle(quote->author, quote->title,
                              &bottomBoxPrintedHeight);

  // Quote
  this->_display->setCursor(this->_drawAreaX, this->_drawAreaY + 15);
  uint16_t maxiumY =
      (this->_drawAreaY + this->_drawAreaHeight) - bottomBoxPrintedHeight * 2;
  this->_display->setFont(_quoteFont);
...
void DisplayController::showWarning(const char *message) {
  ...
  this->_display->drawBitmap(
      this->_drawAreaX +
          (this->_drawAreaWidth - exclamationTriangleSolidWidth) / 2,
      this->_drawAreaY, (uint8_t *)exclamationTriangleSolidBitmap,
      exclamationTriangleSolidWidth, exclamationTriangleSolidHeight,
      GxEPD_BLACK);

  this->_display->setFont(_boldFont);
  this->_display->setTextColor(GxEPD_BLACK);
  this->_display->setCursor(
      this->_drawAreaX,
      this->_drawAreaY + 2 * exclamationTriangleSolidHeight + this->_padding);
  this->_printTextWithBreaksAtSpaces(message,
                                     this->_drawAreaY + this->_drawAreaHeight);

  this->_display->display();
}

void DisplayController::_printTextWithBreaksAtSpaces(const char *text,
                                                     uint16_t maximumY,
                                                     bool wrap) {
  ...
  while (1) {
    ...
    if (this->_display->getCursorY() >= maximumY) {
      ...
      this->_display->setFont(_quoteFont);
      this->_display->fillRect(previousX - previousWidth,
                               previousY - previousHeight, previousWidth + 2,
                               previousHeight * 1.5, GxEPD_WHITE);
...
void DisplayController::_printAuothorAndTitle(const char *author,
                                              const char *title,
                                              uint16_t *printedHeight) {
  ...
  *printedHeight = 0;
  ...
  for (int i = 0; i < 2; i++) {
    ...
    *printedHeight += height;
    int16_t x = MAX((this->_drawAreaX + this->_drawAreaWidth) - width,
                    this->_drawAreaX);
    uint16_t y = 0;
    if (i == 0)
    ...
    this->_display->fillRect(this->_drawAreaX, y - this->_padding,
                             this->_drawAreaWidth + 2,
                             height + this->_padding + 2, GxEPD_WHITE);
    this->_display->setCursor(x, y + height * 0.75);
    this->_display->print(trimmedString);
  }
}

uint16_t DisplayController::_getWidthOfString(const char *string, int16_t x,
                                              int16_t y) {
  int16_t x1, y1;
  uint16_t w, h;
...
uint16_t DisplayController::_getHeightOfString(const char *string, int16_t x,
                                               int16_t y) {
  int16_t x1, y1;
  uint16_t w, h;
  this->_display->getTextBounds(string, x, y, &x1, &y1, &w, &h);
